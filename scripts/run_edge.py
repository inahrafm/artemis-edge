...
"""
scripts/run_edge.py — ARTEMIS v2 Edge Node Entry Point v2.2
Fix v2.2:
  - BUG1: offloader.close() dipanggil sebelum merge_final → stats tidak bisa diakses → file kosong
  - BUG2: checkpoint dihapus di dalam merge_final sebelum write selesai → data hilang jika write gagal
  - BUG3: merge_final tidak ada try/except → exception saat write tidak tertangkap
  - BUG4: offloader.measure_rtt() tidak ada di Offloader → AttributeError diam-diam
  - BUG5: notif_kw di-pass ke semua fn_args tapi node.py lama tidak kenal parameter ini
"""
...
def save_checkpoint(out_dir, exp_id, method_key, data, cfg, args):
    """Save checkpoint dengan atomic write — tulis ke .tmp dulu, rename setelah selesai."""
    cp      = _cp_path(out_dir, exp_id, method_key)
    cp_tmp  = cp.with_suffix(".tmp")
    try:
        with open(cp_tmp, "w") as f:
            json.dump({
                "metadata": {
        ...
        cp_tmp.rename(cp)   # atomic rename — hanya terjadi setelah write selesai
        log.info(f"Checkpoint saved → {cp.name}")
    except Exception as e:
        log.error(f"GAGAL save checkpoint {method_key}: {e}")
        if cp_tmp.exists():
            cp_tmp.unlink()
...
def merge_final(out_dir, exp_id, all_results, cfg, args, offloader_stats):
    """
    FIX BUG1+BUG2+BUG3:
    - Terima offloader_stats sebagai dict (bukan objek offloader) supaya tidak
      bergantung pada offloader yang sudah di-close
    - Tulis ke .tmp dulu, rename setelah selesai (atomic)
    - Hapus checkpoint HANYA setelah rename berhasil
    """
        ...
        with open(out_tmp, "w") as f:
            json.dump({
                "metadata": {
                    ...
                    "offloader_stats": offloader_stats,  # sudah dict, bukan objek
                },
                "methods": all_results,
            }, f, indent=2)

        out_tmp.rename(out_path)  # atomic rename
        log.info(f"Final JSON tersimpan → {out_path.name} ({out_path.stat().st_size:,} bytes)")

    except Exception as e:
        log.error(f"GAGAL menyimpan final JSON: {e}")
        if out_tmp.exists():
            out_tmp.unlink()
        # Checkpoint TIDAK dihapus supaya data bisa di-recover
        raise

    # Hapus checkpoint HANYA setelah final JSON berhasil disimpan
    for mk in ["device_only", "server_only",
                "static_cooperative", "adaptive_cooperative"]:
        cp = _cp_path(out_dir, exp_id, mk)
        if cp.exists():
...
def measure_rtt_ping(server_url: str, n: int = 5) -> float:
    """
    FIX BUG4: offloader tidak punya measure_rtt().
    Ukur RTT via ping ke host dari server_url.
    Return avg RTT dalam ms, atau None jika gagal.
    """
    import re, subprocess
    try:
        host = server_url.split("://")[1].split(":")[0].split("/")[0]
        result = subprocess.run(
            ["ping", "-c", str(n), "-W", "3", host],
        ...
        log.warning(f"RTT ping gagal: {e}")
    return None
...
def main():
    parser = argparse.ArgumentParser(description="ARTEMIS v2 — Edge Node v2.2")
    ...
    parser.add_argument("--rtt_threshold",   default=500.0, type=float,
                        help="Auto-skip server_only jika RTT ping avg > threshold ms")
    ...
    # FIX BUG4: gunakan ping bukan offloader.measure_rtt()
    if server_ok and "server_only" in methods:
        rtt = measure_rtt_ping(cfg.server_url, n=5)
        if rtt and rtt > args.rtt_threshold:
            log.warning(f"RTT ping {rtt:.0f}ms > threshold {args.rtt_threshold:.0f}ms "
                        f"— auto-skip server_only")
            methods = [m for m in methods if m != "server_only"]
        elif rtt:
            log.info(f"RTT ping avg: {rtt:.0f}ms (threshold: {args.rtt_threshold:.0f}ms) — OK")
        else:
            log.warning("RTT ping tidak bisa diukur — lanjut tanpa auto-skip")
    ...
    # FIX BUG1: ambil stats SEBELUM close, simpan sebagai dict
    offloader_stats = offloader.stats
    offloader.close()

    # FIX BUG2+BUG3: merge_final pakai atomic write, hapus checkpoint setelah write berhasil
    if all_results:
        try:
            out_path = merge_final(out_dir, exp_id, all_results, cfg, args,
                                   offloader_stats)
            ...
            if size < 1000:
                log.error(f"WARNING: File hasil sangat kecil ({size} bytes) — mungkin corrupt!")
            else:
                log.info(f"File hasil OK: {size:,} bytes")
        except Exception as e:
            log.error(f"GAGAL menyimpan hasil: {e}")
            log.error("Checkpoint files masih ada — jalankan ulang dengan --resume")
            sys.exit(1)
    else:
